				...
				<div class="footNum">
					<ul>
						<c:choose>
							...
							<c:otherwise>
								<li class="pre"><a
									href="${pageContext.request.contextPath}/toproducttypepage?page=${pagebean.page-1}&typename=${typename}">
										上一页</a></li>
							</c:otherwise>
						</c:choose>
						<c:forEach begin="1" end="${pagebean.pages}" step="1" var="index">
							<c:choose>
								...
								<c:otherwise>
									<li class="num"><a
										href="${pageContext.request.contextPath}/toproducttypepage?page=${index}&typename=${typename}">${index}</a></li>
								</c:otherwise>
							</c:choose>
						</c:forEach>
						<c:choose>
